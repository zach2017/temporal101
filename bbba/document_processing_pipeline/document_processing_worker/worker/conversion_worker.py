"""Worker – Document Conversion. Listens on ``document-conversion-queue``."""
...
async def main() -> None:
    logger.info("worker.conversion.starting", task_queue=settings.temporal_conversion_task_queue)
    client = await Client.connect(settings.temporal_host, namespace=settings.temporal_namespace)
    worker = Worker(
        client, task_queue=settings.temporal_conversion_task_queue,
        workflows=[DocumentConversionWorkflow],
        activities=[convert_document_to_text, store_extracted_text],

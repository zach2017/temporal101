package demo.temporal.workflow;
 ...
 *
 * <p>
 * This interface lives in the <b>common</b> module so that both the Worker
 * (which provides the implementation) and the Client (which creates typed
 * stubs) share the same contract.</p>
 *
...
public interface FileProcessingWorkflow {
    ...
    /**
     * Query: returns a human-readable status string such as "DETECTING_MIME",
     * "EXTRACTING_TEXT", "RUNNING_OCR", "COMPLETED".
     */
    @QueryMethod
    String getStatus();

    /**
     * Signal: allows an external caller to request cancellation. The Workflow
     * will finish its current Activity and then stop.
     */
    @SignalMethod

 ...
 *
 * <p>Uses Temporal's {@link TestWorkflowEnvironment} — an in-memory
 * Temporal server.  All Activities are mocked so we test only the
 * Workflow orchestration logic.</p>
 */
class FileProcessingWorkflowTest {
    ...
    void setUp() {
        ...
        detectionActivities = mock(FileDetectionActivities.class);
        textActivities      = mock(TextExtractionActivities.class);
        ocrActivities       = mock(OcrActivities.class);
        storageActivities   = mock(FileStorageActivities.class);
